...
build_aen() {
    ...
    if [ $GOOS == "windows" ]; then
      go build -o "dist/aen.exe" -ldflags "-X main.Version=$VERSION" -trimpath ./cmd/*.go > /dev/null
      cd dist && 7z a aen-$VERSION-$GOOS-$GOARCH.zip aen.exe > /dev/null 2>&1 && cd ..
      rm dist/aen.exe
    else
      go build -o "dist/aen" -ldflags "-X main.Version=$VERSION" -trimpath ./cmd/*.go > /dev/null
